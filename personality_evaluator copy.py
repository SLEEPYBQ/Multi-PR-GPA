...
"""
        ...
        # ALL条件专用提示词模板
        self.all_condition_prompt_template = PromptTemplate(
            input_variables=["dialogue"],
            template="""
### Background:  
You are a professional personality psychologist specializing in the Big Five personality traits model. You've been invited to analyze the personality traits of a human player based on their interactions with FIVE DIFFERENT AI agents in "Prisoner's Dilemma" games. Each AI agent represents a different personality type (Extraversion, Agreeableness, Conscientiousness, Openness, Neuroticism), and the human player has engaged in complete multi-round dialogues with each agent.

### Task:
1. You are to analyze the human player's personality traits based on their comprehensive dialogue records with all five agents. You will provide a detailed analysis of each of the Big Five personality traits, considering how the human player's behavior patterns, communication styles, and decision-making strategies vary or remain consistent across different agent interactions.
2. Your response should strictly follow the Response Template.

### Analysis Focus for ALL Condition:
- **Cross-Agent Consistency**: How does the player's behavior remain consistent across different agent interactions?
- **Adaptive Responses**: How does the player adapt their communication style and strategies when interacting with different personality types?
- **Personality Manifestation**: Which personality traits emerge most clearly when considering the full spectrum of interactions?
- **Contextual Variation**: How do different game contexts (with different agent personalities) reveal different aspects of the player's personality?

### Big Five Personality Traits Reference Standards: 
#### Openness: 
- High Scores (3.1-5.0): Shows curiosity about different interaction styles, adapts communication approaches with different agents, demonstrates creative problem-solving across contexts
- Medium Scores (3.0): Maintains balance between adaptability and consistency across agent interactions
- Low Scores (1.0-2.9): Uses rigid communication patterns regardless of agent type, resistant to adjusting strategies

#### Conscientiousness: 
- High Scores (3.1-5.0): Maintains consistent ethical standards across all interactions, shows planning and deliberation in responses to different agents
- Medium Scores (3.0): Shows some consistency in approach while allowing situational flexibility
- Low Scores (1.0-2.9): Inconsistent behavior patterns, impulsive responses varying significantly across agent types

#### Extraversion: 
- High Scores (3.1-5.0): Actively engages with all agent types, maintains high energy and sociability across interactions
- Medium Scores (3.0): Shows varied social engagement depending on agent personality type
- Low Scores (1.0-2.9): Reserved interaction style across all agent types, minimal social engagement

#### Agreeableness: 
- High Scores (3.1-5.0): Consistently cooperative and trusting across different agent interactions, shows empathy regardless of agent personality
- Medium Scores (3.0): Selectively cooperative based on agent behavior and context
- Low Scores (1.0-2.9): Consistently skeptical or competitive across all agent types

#### Neuroticism: 
- High Scores (3.1-5.0): Shows emotional volatility or stress patterns across multiple agent interactions
- Medium Scores (3.0): Moderate emotional responses with some variation by context
- Low Scores (1.0-2.9): Maintains emotional stability across all different agent interactions

### Rating Criteria:
1.0-1.9: Very low - Rarely if ever displays characteristics associated with this trait across any agent interaction
2.0-2.7: Low - Occasionally displays characteristics associated with this trait across agent interactions
2.8-3.2: Average - Shows balanced or moderate expression of this trait across different contexts
3.3-4.0: High - Frequently displays characteristics associated with this trait across most agent interactions
4.1-5.0: Very high - Strongly and consistently displays characteristics associated with this trait across all agent interactions

### Analysis Requirements: 
1. Carefully read all dialogue records from interactions with five different agents (Extraversion, Agreeableness, Conscientiousness, Openness, Neuroticism agents).
2. Compare and contrast the player's behavior patterns across different agent interactions.
3. Rate the human player on each dimension considering the full spectrum of their interactions.
4. Base your ratings on specific evidence from multiple dialogue contexts.
5. Quote original text from different agent interactions as supporting evidence.
6. Provide at least 2-3 specific examples from different agent interactions for each dimension's rating.
7. Consider both consistent traits and adaptive behaviors in your analysis.
...
### Response Template: 
### My step by step thought process: 
{{Detailed explanation of how you analyzed each dimension across all five agent interactions, including key behavioral patterns and dialogue examples from different contexts}} 

### Player's Personality Traits Rating: 
- Openness: {{Rating}}, reason: {{Detailed analysis based on specific dialogue content from multiple agent interactions, at least 2-3 examples}} 
- Conscientiousness: {{Rating}}, reason: {{Detailed analysis based on specific dialogue content from multiple agent interactions, at least 2-3 examples}} 
- Extraversion: {{Rating}}, reason: {{Detailed analysis based on specific dialogue content from multiple agent interactions, at least 2-3 examples}} 
- Agreeableness: {{Rating}}, reason: {{Detailed analysis based on specific dialogue content from multiple agent interactions, at least 2-3 examples}} 
- Neuroticism: {{Rating}}, reason: {{Detailed analysis based on specific dialogue content from multiple agent interactions, at least 2-3 examples}}

### Comprehensive Game Dialogue Records from Five Different Agents: 
{dialogue}
"""
        )
    ...
    def evaluate_personality_all_condition(self, dialogue_text):
        """专门用于ALL条件的人格评估方法
        
        Args:
            dialogue_text: 包含与5种不同agent交互记录的完整对话文本
            
        Returns:
            dict: 评估结果，包含ratings, reasons, thought_process, raw_response
        """
        if not self.chat_model:
            print("No valid LLM configured, returning mock result.")
            return self._get_mock_evaluation()
        ...
        try:
            chain = LLMChain(prompt=self.all_condition_prompt_template, llm=self.chat_model)
            response = chain.invoke({"dialogue": dialogue_text})
            ...
            print(f"Exception during ALL condition LLM evaluation: {e}")
            return self._get_mock_evaluation()
